...
class minhaClasse
{
    ...
    static void Main(string[] args)
    {
        ...
        int mul4 = 0;
        int mul5 = 0;
        string[] listaString = Console.ReadLine().Split();
        int[] listaInt = Array.ConvertAll(listaString, int.Parse);

        for (var i = 0; i < tamanhoLista;)
        {
            foreach (var numero in listaInt)
            {
                if (numero % 2 == 0)
                {
                    mul2++;
        ...
        Console.WriteLine("{0} Multiplo(s) de 2", mul2);
        Console.WriteLine("{0} Multiplo(s) de 3", mul3);
        Console.WriteLine("{0} Multiplo(s) de 4", mul4);
        Console.WriteLine("{0} Multiplo(s) de 5", mul5);
